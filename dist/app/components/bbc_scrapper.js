...
export class BBC {
    ...
    async _scarpe() {
        ...
        let FirstRows = await this._client.document.querySelectorAll("div.gel-layout__item.nw-c-top-stories__secondary-item").$map(async (item) => {
        ...
        let FirstRows2 = await this._client.document.querySelectorAll("div.gel-layout__item.nw-c-top-stories__tertiary-items").$map(async (item) => {
            let headline = await item.querySelector("a");
            let excerpt = await item.querySelector("p");
            let category = await item.querySelector("li:nth-child(2)");
            let published = await item.querySelector("li > span > time > span");
    ...
    async _getAuthors() {
        ...
        for (var article of this._payload) {
            ...
            let author = await this._client.querySelector("#main-content > div.ssrcss-1ocoo3l-Wrap.e42f8511 > div > div.ssrcss-rgov1k-MainColumn.e1sbfw0p0 > article > div.ssrcss-1bdte2-BylineComponentWrapper.e8mq1e90 > div > div > div.ssrcss-1dtr1ls-Container-ContributorDetails.e8mq1e913 > div.ssrcss-68pt20-Text-TextContributorName.e8mq1e96");
            if (author === null) {
                article.author = null;
            }
            else {
